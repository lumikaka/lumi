...
test('topbar, persistent sidebar and group tabs preserve the existing routes', () => {
  ...
  assert.match(sidebarSource, /stableRecentProjects\.map\(\(project, index\)/)
  assert.doesNotMatch(sidebarSource, /stableRecentProjects\.slice\(/)
  assert.match(sidebarSource, /PROJECT_TONES\[index % PROJECT_TONES\.length\]/)
  assert.match(sidebarSource, /onContextMenu=\{\(event\) => openProjectContextMenu\(event, project\)\}/)
  assert.match(sidebarSource, /<ProjectActionsMenu[\s\S]*project-index-menu--context/)
  assert.match(sidebarSource, /projectRowActions\(projectContextMenu\.project\)/)
