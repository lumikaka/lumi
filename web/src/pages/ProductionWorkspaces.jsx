...
import { formatTerminologyKey, formatTerminologyMessageKey, readImageFileDimensions } from './pictureBookProfile.js'
import LocalizedErrorMessage from '../i18n/LocalizedErrorMessage.jsx'
import { useI18n } from '../i18n/useI18n.js'
import { comicStateLabel, sourceTypeLabel, statusLabel, taskKindLabel } from '../i18n/labels.js'
import { projectChapterRoute } from '../projectRoutes.js'
...
export function ComicWorkspace({ projectUuid }) {
  ...
  const sectionLabel = (section) => pageMode ? comicPageLabel(t, sections, section) : t('comic.workbench.section_label', { number: section.section_no })
  const sectionFallbackTitle = (section) => pageMode ? comicPageFallbackTitle(t, section) : t('comic.section.untitled')
  const term = (pictureBookKey, verticalStripKey, values) => t(formatTerminologyKey(pictureBook, pictureBookKey, verticalStripKey), values)
  const storyTerm = (key, values) => t(formatTerminologyMessageKey(pictureBook, key), values)
  useEffect(() => { if (!chapterUuid && chaptersQuery.data?.items?.[0]) navigate(projectChapterRoute(projectUuid, chaptersQuery.data.items[0].uuid), { replace: true }) }, [chapterUuid, chaptersQuery.data, navigate, projectUuid])
  useEffect(() => { if (selected && selected.uuid !== selectedUuid) setSelectedUuid(selected.uuid) }, [selected?.uuid, selectedUuid])
  useEffect(() => { if (selected) { setTitle(selected.title || ''); setPageRole(comicPageRole(selected)); setDescription(selected.description_md || ''); setStoryboard(selected.current_storyboard?.content_md || '') } }, [selected?.uuid, selected?.revision])
  const refresh = useCallback(() => {
    ['comic-sections', 'comic-state', 'comic-storyboards', 'comic-images', 'comic-snapshots', 'comic-exports', 'production-tasks']
      .forEach((key) => queryClient.invalidateQueries({ queryKey: [key, projectUuid] }))
    queryClient.invalidateQueries({ queryKey: ['recent-projects'] })
  }, [projectUuid, queryClient])
  const createSection = useMutation({ mutationFn: (role = 'body') => createComicSection(projectUuid, chapterUuid, { title: '', description_md: '', storyboard_md: '', ...(pageMode ? { page_role: role } : {}) }), onSuccess: (item) => { setNewPageRole('body'); setSelectedUuid(item.uuid); refresh() }, onError: setError })
  const saveSection = useMutation({ mutationFn: () => updateComicSection(projectUuid, chapterUuid, selected.uuid, { title, description_md: description, ...(pageMode ? { page_role: pageRole } : {}), expected_revision: selected.revision }), onSuccess: refresh, onError: setError })
  ...
  const moveComicSection = (section, direction) => {
    const source = pageMode ? sections : sections.map((item) => ({ ...item, page_role: 'body' }))
    const next = comicBodyReorderUuids(source, section.uuid, direction)
    if (next) reorder.mutate(next)
  }
  if (projectQuery.isLoading && !projectQuery.data) return <p className="workspace-loading">{t('common.loading')}</p>
  if (projectQuery.isError && !projectQuery.data) return <LocalizedErrorMessage error={projectQuery.error} />
  if (!chapterUuid && chaptersQuery.isLoading) return <p className="workspace-loading">{storyTerm('story.chapter.loading')}</p>
  if (!chapterUuid && !chapterOptions.length) return <div className="workspace-empty"><h2>{term('comic.empty.create_picture_book', 'comic.empty.create_chapter')}</h2><p>{term('comic.empty.picture_book_required', 'comic.empty.chapter_required')}</p></div>
  return <div className="workspace-stack production-workspace comic-workspace">
    <header className="workspace-section-heading"><div><p className="eyebrow">{term('comic.eyebrow_pages', 'comic.eyebrow')}</p><h1>{term('comic.workspace_pages', 'comic.workspace')}</h1><p>{term('comic.description_pages', 'comic.description')}</p></div><div className="comic-header-actions"><select aria-label={term('comic.select_picture_book', 'comic.select_chapter')} value={chapterUuid || ''} onChange={(event) => navigate(projectChapterRoute(projectUuid, event.target.value))}>{chapterOptions.map((chapter) => <option key={chapter.uuid} value={chapter.uuid}>{chapter.chapter_code} · {chapter.title || t('comic.unnamed')}</option>)}</select><button type="button" disabled={bodySections.length > 0 || comicGenerate.isPending || ['queued', 'running'].includes(comicTask?.status)} onClick={() => comicGenerate.mutate()}>{term('comic.generate_page_scripts', 'comic.generate_storyboard')}</button><span className={`comic-state comic-state--${stateQuery.data?.status || 'empty'}`}>{comicTask ? `${statusLabel(t, comicTask.status)} · ${formatNumber(comicTask.progress)}%` : comicStateLabel(t, stateQuery.data?.status || 'empty')}</span></div></header>
    <Notice error={error || sectionsQuery.error} />
    <section className="comic-layout"><aside className={`comic-section-list${pageMode ? ' comic-section-list--pages' : ''}`}><header><h2>{t(pageMode ? 'comic.pages' : 'comic.sections')}</h2>{pageMode ? <select aria-label={t('comic.page_role.create_label')} value={newPageRole} disabled={createSection.isPending} onChange={(event) => setNewPageRole(event.target.value)}>{COMIC_PAGE_ROLES.map((role) => <option key={role} value={role} disabled={comicPageRoleOptionDisabled(sections, role)}>{t(`comic.page_role.${role}_option`)}</option>)}</select> : null}<button type="button" disabled={createSection.isPending || (pageMode && comicPageRoleOptionDisabled(sections, newPageRole))} onClick={() => createSection.mutate(newPageRole)}>{t(pageMode ? 'comic.page.add' : 'comic.section.add')}</button></header>{sections.map((section, index) => {
      const bodyIndex = bodySections.findIndex((item) => item.uuid === section.uuid)
      const fixed = pageMode && comicPageRole(section) !== 'body'
      return <article key={section.uuid} className={section.uuid === selected?.uuid ? 'comic-section-row comic-section-row--active' : 'comic-section-row'}><button type="button" aria-pressed={section.uuid === selected?.uuid} onClick={() => setSelectedUuid(section.uuid)}><span>{sectionLabel(section)}</span><strong>{section.title || sectionFallbackTitle(section)}</strong><small>{t(section.current_image ? 'comic.section.has_image' : section.current_storyboard ? (pageMode ? 'comic.page.has_script' : 'comic.section.has_storyboard') : 'comic.section.draft')}</small></button><div><button type="button" className="button-quiet" disabled={fixed || (pageMode ? bodyIndex === 0 : index === 0) || reorder.isPending} onClick={() => moveComicSection(section, -1)}>↑</button><button type="button" className="button-quiet" disabled={fixed || (pageMode ? bodyIndex === bodySections.length - 1 : index === sections.length - 1) || reorder.isPending} onClick={() => moveComicSection(section, 1)}>↓</button></div></article>
    })}{sections.length === 0 ? <div className="workspace-empty"><h2>{t(pageMode ? 'comic.page.empty' : 'comic.section.empty')}</h2><p>{t(pageMode ? 'comic.page.empty_body' : 'comic.section.empty_body')}</p></div> : null}</aside>
      <div className="comic-editor">{selected ? <><section className="comic-meta-card"><header><p className="eyebrow">{sectionLabel(selected)}</p><button type="button" className="button-quiet danger-text" title={pageMode && selectedPageRole === 'body' && bodySections.length <= 1 ? t('comic.workbench.pages.delete_last_body_title') : undefined} disabled={pageMode && selectedPageRole === 'body' && bodySections.length <= 1} onClick={() => removeSection.mutate()}>{t('common.action.delete')}</button></header><input value={title} onChange={(event) => setTitle(event.target.value)} aria-label={t(pageMode ? 'comic.page.title' : 'comic.section.title')} />{pageMode ? <label>{t('comic.page_role.label')}<select value={pageRole} onChange={(event) => setPageRole(event.target.value)}>{COMIC_PAGE_ROLES.map((role) => <option key={role} value={role} disabled={comicPageRoleOptionDisabled(sections, role, selected.uuid)}>{t(`comic.page_role.${role}_option`)}</option>)}</select></label> : null}<textarea rows="4" value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t('comic.section.description_placeholder')} /><button type="button" disabled={saveSection.isPending || (title === selected.title && description === (selected.description_md || '') && (!pageMode || pageRole === selectedPageRole))} onClick={() => saveSection.mutate()}>{t(pageMode ? 'comic.page.save' : 'comic.section.save')}</button></section>
        <section className="storyboard-editor-card"><header><div><p className="eyebrow">{t(pageMode ? 'comic.page_script' : 'comic.storyboard')}</p><h2>{t(pageMode ? 'comic.page_script.markdown' : 'comic.storyboard.markdown')}</h2></div><button type="button" className="button-secondary" aria-pressed={preview} onClick={() => setPreview((value) => !value)}>{t(preview ? 'comic.storyboard.edit' : 'common.action.preview')}</button></header>{preview ? <MarkdownPreview value={storyboard} /> : <textarea rows="12" value={storyboard} onChange={(event) => setStoryboard(event.target.value)} placeholder={t('comic.storyboard.placeholder')} />}<button type="button" disabled={!storyboard.trim() || saveStoryboard.isPending} onClick={() => saveStoryboard.mutate()}>{t(pageMode ? 'comic.page_script.save_candidate' : 'comic.storyboard.save_candidate')}</button><div className="variant-chip-row">{storyboardsQuery.data?.items?.map((variant) => <button type="button" key={variant.uuid} aria-pressed={selected.current_storyboard?.uuid === variant.uuid} onClick={() => storyboardSelect.mutate(variant)}>v{variant.version_no} · {sourceTypeLabel(t, variant.source_type)}</button>)}</div></section>
        <section className="comic-image-card"><header><div><p className="eyebrow">{t('comic.images.eyebrow')}</p><h2>{t(pageMode ? 'comic.images.page_title' : 'comic.images.title')}</h2><small>{t('comic.images.provider_note')}</small></div>{selected.current_image ? <ProductionImage projectUuid={projectUuid} asset={selected.current_image.asset} alt={selected.title || sectionFallbackTitle(selected)} profile="detail_1024" /> : null}</header>{selected.current_image?.asset ? <ImageRatioNotice pictureBook={pictureBook} width={selected.current_image.asset.width} height={selected.current_image.asset.height} /> : null}<div className="image-action-row"><label>{t('comic.images.replace')}<input type="file" accept="image/*" onChange={(event) => selectImageFile(event.target.files?.[0] || null)} /></label><button type="button" disabled={!imageFile || imageImport.isPending} onClick={() => imageImport.mutate()}>{t('comic.images.import')}</button><button type="button" disabled={!selected.current_storyboard || Boolean(activeTaskFor(tasks, 'comic_image_generation', selected.uuid))} onClick={() => imageGenerate.mutate()}>{t(pageMode ? 'comic.images.generate_page' : 'comic.images.generate')}</button></div>{imageFileDimensions ? <ImageRatioNotice pictureBook={pictureBook} width={imageFileDimensions.width} height={imageFileDimensions.height} beforeImport showCompatible /> : null}<ProductionTaskStrip projectUuid={projectUuid} tasks={tasks} resourceUuid={selected.uuid} kind="comic_image_generation" refresh={refresh} /><div className="image-variant-grid">{imagesQuery.data?.items?.map((variant) => <article key={variant.uuid}><ProductionImage projectUuid={projectUuid} asset={variant.asset} alt={`v${variant.version_no}`} /><ImageRatioNotice pictureBook={pictureBook} width={variant.asset?.width} height={variant.asset?.height} /><span>v{variant.version_no}</span><button type="button" aria-pressed={selected.current_image?.uuid === variant.uuid} onClick={() => imageSelect.mutate(variant)}>{t(selected.current_image?.uuid === variant.uuid ? 'comic.images.current' : 'common.action.restore')}</button></article>)}</div></section></> : <div className="workspace-empty"><h2>{t(pageMode ? 'comic.page.empty' : 'comic.editor.empty')}</h2></div>}</div>
      <aside className="comic-history"><section><header><h2>{t(pageMode ? 'comic.snapshots_pages' : 'comic.snapshots')}</h2><span>{snapshotsQuery.data?.items?.length || 0}</span></header>{snapshotsQuery.data?.items?.slice(0, 12).map((snapshot) => <article key={snapshot.uuid}><strong>v{snapshot.version_no}</strong><span data-machine-value>{snapshot.reason}</span><button type="button" className="button-secondary" onClick={() => navigate(`/projects/${encodeURIComponent(projectUuid)}/chapters/${encodeURIComponent(chapterUuid)}?history=snapshots&snapshot_uuid=${encodeURIComponent(snapshot.uuid)}`)}>{t('comic.workbench.snapshot.preview')}</button></article>)}</section><section><h2>{t('projects.tab.exports')}</h2><button type="button" onClick={() => setExportRequest(comicExportDialogRequest('chapter', chapterUuid, selectedChapterLabel))}>{t(pageMode ? 'comic.exports.picture_book' : 'comic.exports.chapter')}</button><button type="button" className="button-secondary" onClick={() => setExportRequest(comicExportDialogRequest('project'))}>{t('comic.exports.project')}</button>{exportsQuery.data?.items?.slice(0, 6).map((item) => <article key={item.uuid}><strong>{item.scope === 'chapter' ? storyTerm('story.chapter') : t('projects.title')} · {item.format?.toUpperCase() || 'ZIP'} · {statusLabel(t, item.status)}</strong><small>{item.relative_path || item.snapshot_hash.slice(0, 12)}</small><small>{t('projects.exports.retention_days', { days: item.retention_days || 7 })}{item.expires_at ? ` · ${t('projects.exports.expires')} ${formatDateTime(item.expires_at)}` : ''}</small>{item.download_url ? <a href={item.download_url}>{t('comic.exports.open')}</a> : null}</article>)}</section></aside>
    </section>
