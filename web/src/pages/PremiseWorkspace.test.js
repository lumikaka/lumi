...
test('premise narrow layout keeps controls operable and moves ChatArea into an overlay', () => {
  ...
  assert.match(shellStyles, /@media \(max-width: 1199px\)[\s\S]*?grid-template-columns: minmax\(0, 1fr\)/)
  assert.match(layoutSource, /const COMPACT_CHAT_QUERY = '\(max-width: 1199px\)'/)
