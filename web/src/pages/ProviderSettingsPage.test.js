...
test('initial setup keeps the provider list and dialog on the dedicated setup URL', () => {
  ...
  assert.doesNotMatch(appSource, /ProviderGate/)
  assert.match(appSource, /path="\/" element={<HomePage \/>}/)
  assert.match(appSource, /path="\/projects\/:projectUuid\/\*" element={<ProjectActivationGate><StoryWorkspacePage \/><\/ProjectActivationGate>}/)
