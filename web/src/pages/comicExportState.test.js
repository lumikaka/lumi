...
test('all new comic export entry points open the shared operation dialog while history keeps direct downloads', () => {
  ...
  assert.match(dialog, /const \[selectedFormat, setSelectedFormat\] = useState\(''\)/)
  assert.match(dialog, /format,\s*allow_missing_images: allowMissingImages/)
  assert.match(dialog, /else setStage\('select'\)/)
  assert.match(dialog, /aria-pressed=\{selectedFormat === 'zip'\}/)
  assert.match(dialog, /aria-pressed=\{selectedFormat === 'pdf'\}/)
  assert.match(dialog, /disabled=\{!selectedFormat\}/)
  assert.match(dialog, /startExport\(stage === 'confirm'\)/)
  assert.match(dialog, /format: operationFormat/)
  assert.match(dialog, /download_format/)
  assert.doesNotMatch(dialog, /else await startExport\(false\)/)
