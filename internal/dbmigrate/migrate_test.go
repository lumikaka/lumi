...
func TestProjectCreationReferenceAppMigrationPreservesSessionsAndDowngradesWaitingState(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000003); err != nil {
		t.Fatal(err)
	}
...
func TestProjectCreationReferenceFileMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000030); err != nil {
		t.Fatal(err)
	}
	if containsTable(tableNames(t, dsn), "project_creation_reference_files") {
...
func TestProjectCreationReferencePlanAppMigrationPreservesLegacyRowsAndChecksPlans(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000004); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestProjectCreationReferencePlanProjectMigrationPreservesLegacyRowsAndChecksPlans(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260830000033); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestPremiseAssetWorkflowMigrationAddsKindAndRollsBackOnlyItsDedicatedThread(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260831000034); err != nil {
		t.Fatal(err)
	}
...
func TestComicImageBatchWorkflowMigrationPreservesExistingGraphAndDropsOnlyBatchProjection(t *testing.T) {
	dsn := "file:" + filepath.Join(t.TempDir(), "comic-image-batch-workflows.sqlite") + "?_pragma=foreign_keys(1)"
	runner, err := OpenProject(dsn)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := runner.migrator.Migrate(20260903000036); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
	...
	var existingGraph, batchGraph, conversationThread, dedicatedThread int
	if err := db.QueryRow(`SELECT
		(SELECT COUNT(*) FROM workflows WHERE uuid=?)+
		(SELECT COUNT(*) FROM workflow_steps WHERE uuid=?)+
		(SELECT COUNT(*) FROM workflow_events WHERE uuid=?)+
		(SELECT COUNT(*) FROM workflow_awaits WHERE uuid=?)+
		(SELECT COUNT(*) FROM llm_logs WHERE uuid=? AND workflow_id=1 AND workflow_step_id=1)`, existingWorkflowUUID, existingStepUUID, existingEventUUID, existingAwaitUUID, existingLogUUID).Scan(&existingGraph); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRow(`SELECT
		(SELECT COUNT(*) FROM workflows WHERE uuid IN (?,?))+
		(SELECT COUNT(*) FROM workflow_steps WHERE uuid IN (?,?))+
		(SELECT COUNT(*) FROM workflow_awaits WHERE uuid=?)`, batchWorkflowUUID, inlineWorkflowUUID, batchStepUUID, inlineStepUUID, batchAwaitUUID).Scan(&batchGraph); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM chat_threads WHERE uuid=? AND thread_type='conversation'`, conversationUUID).Scan(&conversationThread); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM chat_threads WHERE uuid=?`, batchThreadUUID).Scan(&dedicatedThread); err != nil {
		t.Fatal(err)
	}
	if existingGraph != 5 || batchGraph != 0 || conversationThread != 1 || dedicatedThread != 0 {
		t.Fatalf("down migration existing=%d batch=%d conversation=%d dedicated=%d", existingGraph, batchGraph, conversationThread, dedicatedThread)
	}
	if _, err := db.Exec(`INSERT INTO workflows(uuid,project_id,kind,title,status,input_version,input_snapshot,idempotency_key,provider_uuid,model,model_source,current_step_key,created_at,updated_at) VALUES('01a06a00-0000-7000-8000-000000000025',1,'comic_image_generation_batch','Rejected','queued',1,'{}','rejected-batch-workflow','` + providerUUID + `','model','provider_default','generate_section_image:001','` + now + `','` + now + `')`); err == nil {
		t.Fatal("down migration still accepted comic_image_generation_batch")
	}
	rows, err := db.Query(`PRAGMA foreign_key_check`)
	if err != nil {
		t.Fatal(err)
	...
	if rows.Next() {
		rows.Close()
		t.Fatal("comic image batch workflow down migration left a foreign key violation")
	}
	rows.Close()
	if err := db.Close(); err != nil {
		t.Fatal(err)
	...
	defer db.Close()
	if _, err := db.Exec(`INSERT INTO workflows(uuid,project_id,kind,title,status,input_version,input_snapshot,idempotency_key,provider_uuid,model,model_source,current_step_key,created_at,updated_at) VALUES('01a06a00-0000-7000-8000-000000000026',1,'comic_image_generation_batch','Accepted','queued',1,'{}','accepted-batch-workflow','` + providerUUID + `','model','provider_default','generate_section_image:001','` + now + `','` + now + `')`); err != nil {
		t.Fatalf("re-up rejected comic_image_generation_batch: %v", err)
	}
}

func TestProjectSetupGenerationBriefMigrationBackfillsAndRoundTrips(t *testing.T) {
	dsn := "file:" + filepath.Join(t.TempDir(), "project-setup-generation-brief.sqlite") + "?_pragma=foreign_keys(1)"
	runner, err := OpenProject(dsn)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := runner.migrator.Migrate(20260901000035); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
	...
	t.Cleanup(func() { _ = db.Close() })
	if tableHasColumn(t, db, "project_setup_drafts", "generation_brief") {
		t.Fatal("generation_brief survived rollback")
	}
	const now = "2026-09-03T00:00:00Z"
	originalInput := "  " + strings.Repeat("月", 4100) + "  "
	if _, err := db.Exec(`INSERT INTO projects(id,uuid,name,format_version,schema_version,setup_status,created_at,updated_at) VALUES(1,'01a06500-0000-7000-8000-000000000001','Brief migration',1,35,'draft',?,?)`, now, now); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`INSERT INTO project_setup_drafts(uuid,project_id,status,revision,original_input,generation_language,field_sources_json,missing_fields_json,error_code,error_message,created_at,updated_at) VALUES('01a06500-0000-7000-8000-000000000002',1,'draft',1,?,'zh-Hans','{"generation_language":"system_default"}','[]','','',?,?)`, originalInput, now, now); err != nil {
		t.Fatal(err)
	}
	if err := runner.Up(); err != nil {
		t.Fatal(err)
	}
	var brief, source string
	if err := db.QueryRow(`SELECT generation_brief,json_extract(field_sources_json,'$.generation_brief') FROM project_setup_drafts WHERE project_id=1`).Scan(&brief, &source); err != nil {
		t.Fatal(err)
	}
	if len([]rune(brief)) != 4000 || source != "system_default" || strings.TrimSpace(brief) != brief {
		t.Fatalf("backfilled brief runes=%d source=%q trimmed=%t", len([]rune(brief)), source, strings.TrimSpace(brief) == brief)
	}
	if _, err := db.Exec(`UPDATE project_setup_drafts SET generation_brief='' WHERE project_id=1`); err == nil {
		t.Fatal("generation_brief constraint accepted an empty value")
	}
	if err := runner.migrator.Migrate(20260901000035); err != nil {
		t.Fatal(err)
	}
	if tableHasColumn(t, db, "project_setup_drafts", "generation_brief") {
		t.Fatal("generation_brief survived second rollback")
	}
	var rows int
	if err := db.QueryRow(`SELECT COUNT(*) FROM project_setup_drafts WHERE project_id=1`).Scan(&rows); err != nil || rows != 1 {
		t.Fatalf("setup draft did not survive rollback: rows=%d err=%v", rows, err)
	}
}
...
func TestComicSectionPageRoleMigrationBackfillsAndConstrainsSpecialPages(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000031); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestComicSectionPremiseAssetSelectionMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260829000032); err != nil {
		t.Fatal(err)
	}
	if containsTable(tableNames(t, dsn), "comic_section_premise_asset_selections") {
...
func TestProjectSetupLifecycleMigrationKeepsExistingProjectsReady(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000029); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestChapterChatReferenceMigrationUsesUUIDOnlyInFinalSchema(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260828000028); err != nil {
		t.Fatal(err)
	}
	if tableHasColumn(t, db, "chat_context_references", "chapter_id") {
...
func TestChapterChatReferenceMigrationPreservesUUIDWithoutChapterForeignKey(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260825000024); err == nil {
...
func TestLongRunningChatTurnMigrationBackfillsLegacyStepsAndRoundTrips(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260826000026); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestVersionedUserInputRequestMigrationPreservesLegacyAndRejectsLossyDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260826000025); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestPictureBookProfileMigrationBackfillsPreProfileProjectsAndIsImmutable(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260811000018); err != nil {
		t.Fatal(err)
	}
...
func TestProjectModelSettingsMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260811000016); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
...
func TestProjectChatContextReferenceMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260815000023); err != nil {
		t.Fatal(err)
	}
...
func TestAssetStoreMigrationDownRemovesSchemaCleanly(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260807000002); err != nil {
		t.Fatal(err)
	}
...
func TestComicSectionPremiseMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260810000012); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
...
func TestComicImageWorkflowMigrationPreservesExistingYoloGraph(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260810000014); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestPremiseComicMigrationDownKeepsAssetStore(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260807000004); err != nil {
		t.Fatal(err)
	}
	if err := runner.Close(); err != nil {
		t.Fatal(err)
...
func TestPremiseChatParityMigrationPreservesExistingRowsAndForeignKeys(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260809000007); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestStoryPromptWorkflowMigrationPreservesAuditAndAgentPromptHistory(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260809000009); err != nil {
		t.Fatal(err)
	}
...
func TestUnifiedAICallLogMigrationPreservesStoryAndChatRows(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260809000010); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestLLMLogPayloadMigrationPreservesLegacyRowsAndJSONConstraints(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260810000011); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestExpandedPromptEditingMigrationCanonicalizesLegacyHistoryAndRoundTripsRuntime(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260810000013); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestLLMUsageMetricsMigrationUpAndDown(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260811000017); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
...
func TestStoryChapterWorkflowMigrationPreservesExistingGraphAndDropsOnlyProjection(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260812000020); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
...
func TestComicStoryboardWorkflowMigrationPreservesGraphAndRollsBackKind(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260812000019); err != nil {
		t.Fatal(err)
	}
	db, err = sql.Open("sqlite", dsn)
	if err != nil {
...
func TestComicExportRetentionMigrationBackfillsAndRollsBack(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260814000021); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
...
func TestComicPDFExportMigrationPreservesZIPAndRollsBackPDFRows(t *testing.T) {
	...
	if err := runner.migrator.Migrate(20260814000022); err != nil {
		t.Fatal(err)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
