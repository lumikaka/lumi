...
func agentDefinitions(language string) []Definition {
	...
	meta := func(key, zhTitle, zhDescription, enTitle, enDescription, value string) Definition {
		if english {
	...
	return []Definition{
		...
		meta("premise_asset_scene", "Premise 单项场景", "单设定项生成 Agent 的工具调用语义。", "Premise asset scene", "Tool-use semantics for single premise-asset generation.", choose(agentPremiseAssetSceneZH, agentPremiseAssetSceneEN)),
		meta("asset_reference_scene", "设定项引用场景", "修改一个公开 UUID 设定项，或基于它创建新设定项。", "Asset reference scene", "Edit one public-UUID premise asset or derive a new asset from it.", choose(agentAssetReferenceSceneZH, agentAssetReferenceSceneEN)),
