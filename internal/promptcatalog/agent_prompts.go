...
const (
	...
	currentAgentImageResultRuleZH = "- 使用 `image_gen` 的新 File 成功创建或更新设定资产后，对应 Tool Result 会保存该资产更新后的冻结 Reference；后续继续编辑时选择该资产的资源 UUID。`image_gen` 成功只证明图片文件已生成，不证明画面符合要求；没有用户反馈或可验证的工具错误时，不得凭猜测自行重复生成或写回。"
...
func previousAgentBaseDefaults(value, language string) []string {
	...
	currentImageResult := currentAgentImageResultRuleZH
	if language == LanguageEnglish {
		...
		currentImageResult = currentAgentImageResultRuleEN
	}
	previousImageResultRule := strings.Replace(value, currentImageResult+"\n", "", 1)
	...
	return []string{previousImageResultRule, previousReferenceRule, previousBootstrapRule, previousInputRule, olderInputRule, legacyBootstrap, withoutBootstrap, previousUIRef, legacyInputRule, strings.Replace(legacyInputRule, currentGuide, previousGuide, 1)}
}
