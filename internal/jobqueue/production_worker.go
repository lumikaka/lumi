...
func (runtime *projectRuntime) callProductionText(ctx context.Context, record productionTaskRecord, snapshot production.GenerationSnapshot, resolved provider.Resolved, scenario string, request llm.Request) (llm.Response, error) {
	...
	handle, err := llmlog.Begin(ctx, runtime.store, runtime.manager.hub, llmlog.StartInput{Prices: runtime.manager.providers.Prices(), Endpoint: request.BaseURL,
	...
	finishErr := llmlog.Finish(context.WithoutCancel(ctx), runtime.store, runtime.manager.hub, handle, llmlog.FinishInput{BillingUsage: response.BillingUsage(),
...
func (runtime *projectRuntime) callProductionImage(ctx context.Context, record productionTaskRecord, snapshot production.GenerationSnapshot, resolved provider.Resolved, scenario string, request imagegen.Request) (imagegen.Response, error) {
	...
	finishErr := llmlog.Finish(context.WithoutCancel(ctx), runtime.store, runtime.manager.hub, handle, llmlog.FinishInput{BillingUsage: response.BillingUsage(), OutputSummary: outputSummary, Response: responsePayload, Err: callErr})
	if finishErr != nil {
		if callErr != nil {
			callErr = errors.Join(callErr, finishErr)
		} else {
