...
func (worker *storyGenerationWorker) Work(ctx context.Context, job *river.Job[riverArgs]) error {
	...
	if strings.TrimSpace(response.Content) == "" {
		...
		logHandle, logErr := llmlog.Begin(workCtx, runtime.store, runtime.manager.hub, llmlog.StartInput{Prices: runtime.manager.providers.Prices(), Endpoint: request.BaseURL,
		...
		finishErr := llmlog.Finish(context.WithoutCancel(ctx), runtime.store, runtime.manager.hub, logHandle, llmlog.FinishInput{BillingUsage: response.BillingUsage(),
