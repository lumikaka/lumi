...
func New(cfg config.Config, appStore *appstore.Store, projects *project.Manager) (*Application, error) {
	...
	projectSetupHandler := httpapi.NewProjectSetupHandler(projects, modelResolver, realtimeHub)
	...
	api.GET("/model-settings", modelSettingsHandler.ShowGlobal)
	api.PATCH("/model-settings", modelSettingsHandler.UpdateGlobal)
