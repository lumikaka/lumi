...
func projectSetupDraftUpdateBodySchema() map[string]any {
	...
	return apiObject(map[string]any{
		...
		"generation_brief":    apiLimitedString("基于原始需求、用户补充和明确展示建议整理的生成 Brief。", 4000),
...
func projectSetupReferenceUpdateBodySchema() map[string]any {
	return apiObject(map[string]any{
		...
		"include_in_yolo":   apiBoolean("是否让该图参与本次自动视觉生成。"),
	}, "expected_revision")
}
...
func phase3AgentAPIProjectors() []agentAPIProjector {
	...
	return []agentAPIProjector{
		...
		{Key: "comic_image_generation_batch", Fields: fields("workflow_uuid", "chapter_uuid", "requested_count", "accepted_count", "tasks"), RecommendedFields: []string{"workflow_uuid", "chapter_uuid", "requested_count", "accepted_count", "tasks"}},
