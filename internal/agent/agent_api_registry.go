...
func rawAgentAPIRoutes() []agentAPIRoute {
	...
	base := []agentAPIRoute{
		{ID: RouteProjectSetupGet, Action: "读取项目初始化草稿", Method: "GET", PathTemplate: project + "/project-setup", Handler: routeProjectAPIDispatch, Projector: "project_setup", DocPath: projectSetupDocPath, RecommendedResponseFilter: ".data | {uuid,project_uuid,setup_status,status,revision,draft_values,field_sources,missing_information,final_picture_book,updated_at}", ReadOnly: true, StrictSchema: true, Risk: RiskLow},
