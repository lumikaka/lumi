...
func rawAgentAPIProjectors() []agentAPIProjector {
	base := []agentAPIProjector{
		...
		{Key: "comic_section", Fields: []agentAPIResponseField{
			...
			{Name: "section_no", Type: "integer", Description: "Section 在章节内的绝对装订顺序。"},
			{Name: "page_role", Type: "string", Description: "页面角色：front_cover、body 或 back_cover。"},
			...
			{Name: "revision", Type: "integer", Description: "当前乐观并发版本。"},
		}, RecommendedFields: []string{"uuid", "chapter_uuid", "section_no", "page_role", "title", "revision"}},
...
func rawAgentAPIRoutes() []agentAPIRoute {
	...
	base := []agentAPIRoute{
		{ID: RouteProjectSetupGet, Action: "读取项目初始化草稿", Method: "GET", PathTemplate: project + "/project-setup", Handler: routeProjectAPIDispatch, Projector: "project_setup", DocPath: projectSetupDocPath, RecommendedResponseFilter: ".data | {uuid,project_uuid,setup_status,status,revision,draft_values,field_sources,missing_information,final_picture_book,reference_plan,updated_at}", ReadOnly: true, StrictSchema: true, Risk: RiskLow},
		{ID: RouteProjectSetupUpdate, Action: "更新项目初始化草稿", Method: "PATCH", PathTemplate: project + "/project-setup", Handler: routeProjectAPIDispatch, Projector: "project_setup", DocPath: projectSetupDocPath, BodySchema: projectSetupDraftUpdateBodySchema(), ExpectedRevision: true, StrictSchema: true, Risk: RiskWrite},
		{ID: RouteProjectSetupReferenceUpdate, Action: "更新项目初始化参考图计划", Method: "PATCH", PathTemplate: project + "/project-setup/references/{reference_uuid}", Handler: RouteProjectSetupReferenceUpdate, Projector: "project_setup", DocPath: projectSetupDocPath, BodySchema: projectSetupReferenceUpdateBodySchema(), ExpectedRevision: true, StrictSchema: true, Risk: RiskWrite},
