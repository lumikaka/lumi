...
func TestCurrentProjectAPIToolRejectsRoutesBodiesAndInvalidFiles(t *testing.T) {
	...
	if _, err := validateToolArguments(currentProjectAPIToolName, validRaw); err != nil {
	...
	for _, raw := range []string{
		...
		if _, err := validateToolArguments(currentProjectAPIToolName, raw); err == nil {
...
func TestCurrentProjectAPIToolIntentAuditsSubjectAndLegacyPromptUpgrades(t *testing.T) {
	harness := newAgentHarness(t)
	ctx := context.Background()
	productionService := production.NewService(harness.store, nil)
	profile, err := productionService.GetPremise(ctx)
	if err != nil {
		t.Fatal(err)
	}
	profile, err = productionService.UpdatePremise(ctx, production.UpdatePremiseInput{DefaultStyle: "靛蓝水彩纸纹、柔和月光与克制的暖金点缀", ExpectedRevision: profile.Revision})
	if err != nil {
		t.Fatal(err)
	}
	upload, err := productionService.Files().CreateUpload(ctx, files.CreateUploadInput{Purpose: "premise_asset", OriginalFilename: "source.png", Reader: bytes.NewReader(agentTestPNG(t))})
	...
	legacy := contextPromptSet{Scene: "Use get_premise_asset first, then update_premise_asset with expected_revision."}
	upgraded, err := upgradeLegacyImageScenePrompt(ctx, harness.store, legacy, promptcatalog.LanguageChinese, tc.Thread)
	if err != nil {
		t.Fatal(err)
	}
	for _, expected := range []string{currentProjectAPIToolName, harness.project.UUID, asset.UUID, asset.Title, asset.CurrentVariant.Asset.UUID, profile.DefaultStyle, "POST", "PATCH", "DELETE"} {
		if !strings.Contains(upgraded.Scene, expected) {
			t.Fatalf("upgraded prompt missing %q: %s", expected, upgraded.Scene)
		}
	}
	if strings.Contains(upgraded.Scene, "get_premise_asset") || strings.Contains(upgraded.Scene, "update_premise_asset") || strings.Contains(upgraded.Scene, "{{") {
		t.Fatalf("legacy typed-tool snapshot was not replaced: %s", upgraded.Scene)
	}
	legacyExecutionUUID := mustAgentUUID(t)
	legacyArguments, _ := json.Marshal(map[string]any{"premise_asset_uuid": asset.UUID, "expected_revision": asset.Revision, "summary": "旧 execution 已恢复"})
	legacyResult, err := harness.service.executeTool(ctx, harness.store, tc, toolExecutionRecord{UUID: legacyExecutionUUID, ToolName: "update_premise_asset", ArgumentsJSON: string(legacyArguments)})
