...
本 Guide 用于从文本、当前消息附件、已有项目图片参考或 ready upload 创建一个新的 Premise Asset。API 字段以 `/api/v1/agent-docs/premise-asset.md` 和 `/api/v1/agent-docs/premise.md` 为准。

## 前置条件

- 已知当前 `project_uuid`、`asset_type`、标题，以及足以生成或导入图片的用户意图。
- 需要遵循项目画风时，先读取当前 Premise 的 `default_style`；需要避免重复时，读取已有设定项列表。
- 图片来源必须在以下四种来源中选择一种：纯文本生成、当前消息附件引导生成、已有项目图片引导生成、ready upload 直接导入。
...
2. 根据图片来源处理：
   - 纯文本：调用 `image_gen`，Prompt 包含完整用户要求和必要的 `default_style`。
   - 当前消息附件：附件会自动传给 `image_gen`，不要向用户索取文件 UUID，也不要在 `reference_file_uuids` 中重复传入。
   - 已有项目图片：将该图片的公开文件 UUID 放入 `image_gen.reference_file_uuids`，生成一张新图片。
...
4. 调用 `POST /api/v1/projects/{project_uuid}/premise-assets`。`file_uuid` 与 `upload_uuid` 必须且只能提供一个：
   - `file_uuid` 必须是当前会话刚刚由 `image_gen` 返回、用途匹配且尚未消费的文件 UUID。
   - `upload_uuid` 必须是当前项目中 ready 且尚未消费的上传 UUID。
5. 只有 POST 返回成功信封后才报告创建完成。

## 禁止捷径

- 已有设定项的 `current_variant.asset.uuid` 只能作为 `image_gen.reference_file_uuids` 的参考输入，不能直接作为 POST 的 `file_uuid`。
- 当前消息附件或任意已有项目文件也不能直接作为 POST 的 `file_uuid`；要么先经 `image_gen` 产生当前会话的新输出，要么使用 ready `upload_uuid`。
...
- `production_validation_failed`：来源、会话或用途不匹配。若使用已有项目图，请把它作为参考重新调用 `image_gen`，再提交新返回的 `file_uuid`。
