...
本 Guide 用于更新已有 Premise Asset 的元数据或图片、从绑定资产派生新设定项，以及在用户明确要求时软删除。API 字段以 `/api/v1/agent-docs/premise-asset.md` 为准。

## 前置条件

- 已知目标 `premise_asset_uuid`；绑定 Subject 是默认操作对象，不是 API 权限边界。
- 每次写操作前都必须重新 GET 目标，读取 `uuid`、元数据、`current_variant` 和最新 `revision`。
- 图片替换必须位于绑定该目标 Premise Asset 的 `asset_reference` 上下文；该上下文会把当前设定图作为第一张自动参考图，随后加入当前消息附件。
...
2. 根据目标选择操作：
   ...
   - 图片替换：在绑定资产上下文调用 `image_gen`；自动参考图不要重复放入 `reference_file_uuids`。将本次调用新返回的、尚未消费的 `file_uuid` 放入 PATCH，并提交最新 `expected_revision`。
...
- 不得把绑定资产当前图片的 UUID 直接写入 PATCH/POST；图片写回只接受当前绑定会话 `image_gen` 的新输出。
- 不得在非绑定资产上下文替换图片，也不得把软删除描述为永久删除。
- 不得在用户只要求读取、解释或生成建议时主动修改或删除资源。

## 失败恢复

- revision conflict：重新 GET，比较用户目标与最新事实，再决定是否基于新 revision 重试。
- `production_validation_failed`：确认当前 thread 绑定的资产就是 PATCH 目标，并重新调用 `image_gen` 获取用途匹配的新文件。
