...
本 Contract 的创建入口只用于首页创建会话的首个 bootstrap Turn；普通 ready 对话和普通 UI 入口不能借用该授权。

## `POST /api/v1/projects/{project_uuid}/workflows`

在已确认的 Setup 上启动受控 YOLO 项目初始化，并把当前 Chat Run 持久挂起至 Workflow 终态。
...
| `story_prompt` | body | 非空字符串，最长 4000 字符 | 是 | 基于 `original_input`、后续回答和已展示建议整理的 YOLO 故事 Brief，不得覆盖原始输入。 |
...
| `data.kind` | 字符串 | Workflow 类型，YOLO 为固定公开 kind。 |
...
- 调用前必须已读取初始化 Guide 和本 Contract；仅同一 bootstrap Run 已消费 `confirm_setup_and_start_yolo`、绑定的 Project Setup finalization 成功且项目事实为 `ready` 时可用。
...
- 服务端按可信 `creation_session_uuid` 绑定幂等键。同一 bootstrap Turn 的重复调用、恢复或应用重启只恢复同一 Workflow 和 `workflow_awaits`，不创建第二个 Workflow、await、Thread 或首步 Job。升级前已有 dedicated YOLO 保持原归属，不在恢复时迁移。
- YOLO 固定完成项目初始化、故事、Story Profile、Premise、正文页规划和初始图片，只创建或复用 `vol01.ch01`。它生成 1～6 个 `body` Section；普通绘本另有 `front_cover`，并默认生成封面与首个正文页成品图；`vertical_strip` 不建封面，只生成首个画面段落成品图；Premise Setting Image 仍是既有步骤。
