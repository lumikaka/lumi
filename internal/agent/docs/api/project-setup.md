...
仅当当前项目的 `setup_status` 为 `draft` 时，使用本 API 从首页原始输入整理候选项目设置。候选不是正式项目事实；必须经过用户明确确认并完成 finalization，项目才会变为 `ready`。

- `GET /api/v1/projects/{project_uuid}/project-setup`
  - 不传 `query` 或 `request_body`。
  - 使用 `.data | {uuid,project_uuid,setup_status,status,revision,candidate,field_sources,missing_information,final_picture_book,updated_at}`。
  - `candidate` 包含 `project_name`、`generation_language`、`overall_style` 与 `picture_book`。
...
- `PATCH /api/v1/projects/{project_uuid}/project-setup`
  ...
  - 至少提交一个候选字段：`project_name`、`generation_language`、`overall_style`、`picture_book`。
...
- `POST /api/v1/projects/{project_uuid}/project-setup-finalizations`
  ...
  - 这是危险操作：先向用户清楚展示全部候选值、默认来源和缺失信息，再按确认协议请求一次明确确认。
