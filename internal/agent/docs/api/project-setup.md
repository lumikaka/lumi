...
```json
{
  ...
  "response_filter": ".data | {uuid,project_uuid,setup_status,status,revision,draft_values,field_sources,missing_information,final_picture_book,updated_at}"
}
```
...
- 危险操作；先清楚展示全部待确认设置、默认来源和缺失信息，再按 Overview 的确认协议处理完整请求。
