...
项目模块负责本地项目的创建、打开、关闭、最近项目索引、对话式草稿初始化和项目级创作配置。项目是项目库中其他业务资源的归属边界；Story 总纲、绘本规格和 Prompt 覆盖都依附于项目，但各自保留独立的版本或配置生命周期。
...
首页的一行需求先创建 `setup_status=draft` 的真实项目和普通 Agent 对话。草稿只允许读取事实、继续聊天和维护 Project Setup；Story、图片、Workflow、生产与导出统一以稳定错误码拒绝。Agent 只在必要时补问影响首章的少量问题，并以稳定确认项同时说明“定稿并启动 YOLO”。用户明确选择后，定稿事务写入唯一正式绘本规格并把项目切换为 `ready`；同一 bootstrap Turn 只能幂等启动 existing YOLO，其他生产写入继续失败关闭，后续 Turn 恢复普通 ready 能力。
...
| `对话式项目创建与设置定稿` | [`features/对话式项目创建与设置定稿.md`](features/对话式项目创建与设置定稿.md) | 从首页原始需求可靠创建草稿项目、首个 Agent Turn，并在确认后原子定稿。 |
...
| 对话线程 | 创建 Saga 在草稿项目中恰好一次建立普通 `conversation` Thread、首个 Turn/Run 和原始用户 Item；Agent 通过受控 Setup API 推进候选。 |
