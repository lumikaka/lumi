...
`task_runs`、`production_task_runs`、`chat_threads`、`chat_runs` 与 `workflows` 的 `provider_uuid`、`model` 保存实际执行选择，`model_source` 保存解析来源。Chat Thread 不保存业务 Scene 或 subject；Chat Run 的首个 User Item metadata 冻结 Prompt 内容、`tool_mode` 与 `project_api_v3` 协议，Reference 另存于对应 User Item。迁移前记录使用 `legacy_frozen`，不推断历史来源。任务业务结果分别由章节、项目、漫画或导出 domain 解释。
