...
新创建的系统草稿首次定稿后，按正式项目名称自动命名文件夹。应用库只为新建系统草稿记录一次性标记；历史项目默认不启用，用户自选或导入的目录保持原名。项目从其他路径打开时清除自动命名标记和待执行改名。

首页项目列表、侧边栏右键菜单和项目设置均可修改名称，默认只改显示名称。勾选“同时重命名项目文件夹”后，通过 `GET /api/v1/projects/{project_uuid}/directory-name-preview?name=...` 预览新路径；`PATCH /api/v1/projects/{project_uuid}` 接收可选 `rename_directory` 布尔字段。目录名沿用现有字符规范化与 `-2`、`-3` 至 `-1000` 编号规则，编号不写入显示名称。预览不占用目录，执行时用平台原子的禁止覆盖 rename 再次处理冲突。

目录重命名复用服务端生命周期检查：当前请求和后台任务结束后，停止目标 Runtime、关闭 SQLite、移动整个项目，再打开 Store 和 Runtime。保留项目 UUID、其他已打开项目及原有 WebSocket presence。待执行名称存入应用库，关闭项目后下次打开继续处理；不会为了改目录而主动打开已关闭项目。应用库同时更新最近路径和创建会话的计划路径。若在移动后、更新索引前退出，下次打开只在已登记的目标名称及编号中验证同一项目 UUID 并恢复路径。

名称索引或目录更新后通过现有 `open_project:changed` 提示客户端使查询失效，并由 REST 重读事实状态；没有新增 HTTP 轮询。
