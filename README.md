...
Lumi 把这些环节放回同一个项目：从一个想法开始，逐步完成剧情、章节、角色与场景设定、漫画分镜、图片候选、预览和导出。AI 可以帮你向前推进，但故事怎么讲、哪一版留下、哪个画面采用，始终由你决定。

## 为什么是 Lumi

- **不会画，也能开始做绘本**：先说清楚主角是谁、想完成什么、会经历怎样的变化，Lumi 会帮你建立故事和视觉创作的起点。
- **角色和场景有共同参考**：人物、地点和道具集中保存在设定工作区，生成画面时可以带上选中的参考，不必每一张都从头描述。
- **不满意就改，不必推倒重来**：正文、提示词、分镜和图片都可以保留多个候选版本，随时切换或恢复。
- **从故事到画面不用来回搬运**：章节正文可以直接进入漫画分镜流程，分镜继续连接设定参考和图片生成。
- **项目是你自己的**：每本绘本都保存在你选择的本机文件夹中，可以整体移动、备份和再次打开。
...
2. **完成剧情与章节**：生成或手动编辑剧情简介，创建、续写、批量生成章节，也可以导入已有的 TXT / Markdown 稿件。
...
集中维护剧情简介、章节正文和创作提示。可以手动写作、导入现有章节、AI 续写下一章，或一次规划并生成多个章节。正文与剧情简介都会保留版本，外部修改 `STORY.md` 时也不会被静默覆盖。
...
章节可以拆分成多个画面段落。每个段落都有独立的分镜脚本和图片候选，可以调整标题、顺序、镜头、动作与对白，也支持批量选择分镜生成图片。每次结构和当前画面的变化都会留下可恢复快照。
