...
DROP TABLE story_prompt_results;

-- Preserve every dependent record belonging to the task kind supported by the
-- previous schema. Workflow-only tasks intentionally disappear on rollback,
-- while existing chapter-generation audit history remains intact.
CREATE TEMP TABLE migration_10_task_events AS
SELECT event.* FROM task_events event
JOIN task_runs task ON task.id = event.task_run_id
WHERE task.kind = 'story_chapter_generation';

CREATE TEMP TABLE migration_10_agent_runs AS
SELECT run.* FROM agent_runs run
JOIN task_runs task ON task.id = run.task_run_id
WHERE task.kind = 'story_chapter_generation';

CREATE TEMP TABLE migration_10_agent_events AS
SELECT event.* FROM agent_events event
JOIN agent_runs run ON run.id = event.agent_run_id
JOIN task_runs task ON task.id = run.task_run_id
WHERE task.kind = 'story_chapter_generation';

CREATE TEMP TABLE migration_10_llm_logs AS
SELECT log.* FROM llm_logs log
JOIN task_runs task ON task.id = log.task_run_id
WHERE task.kind = 'story_chapter_generation';

CREATE TEMP TABLE migration_10_story_generation_results AS
SELECT result.* FROM story_generation_results result
JOIN task_runs task ON task.id = result.task_run_id
WHERE task.kind = 'story_chapter_generation';
...
CREATE TABLE task_runs_previous (
...
-- Keep completed chapter generation tasks. Workflow task audit remains in
-- workflow/task events above this schema version and can be exported before a
-- deliberate rollback.
INSERT INTO task_runs_previous
SELECT * FROM task_runs WHERE kind = 'story_chapter_generation';
DROP TABLE task_runs;
ALTER TABLE task_runs_previous RENAME TO task_runs;
...
CREATE TABLE project_prompt_versions_without_agent (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    uuid TEXT NOT NULL UNIQUE,
    project_id INTEGER NOT NULL,
    ...
    CONSTRAINT project_prompt_versions_restore_fk FOREIGN KEY (restored_from_version_id) REFERENCES project_prompt_versions_without_agent(id),
...
-- Preserve Agent prompt history across rollback by namespacing it into the
-- existing premise group. Reapplying this migration restores the Agent group.
INSERT INTO project_prompt_versions_without_agent (
...
SELECT
    ...
    CASE WHEN prompt_group = 'agent' THEN 'premise' ELSE prompt_group END,
    CASE WHEN prompt_group = 'agent' THEN 'agent.' || prompt_key ELSE prompt_key END,
    version_no, prompt, prompt_hash, source_type, created_at
FROM project_prompt_versions;

DROP TABLE project_prompt_versions;
ALTER TABLE project_prompt_versions_without_agent RENAME TO project_prompt_versions;
