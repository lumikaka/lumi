...
CREATE TEMP TABLE premise_asset_workflow_thread_ids AS
SELECT thread_id FROM workflows WHERE kind='premise_asset_generation' AND thread_id IS NOT NULL;
DELETE FROM workflows WHERE kind='premise_asset_generation';
DELETE FROM chat_threads WHERE id IN (SELECT thread_id FROM premise_asset_workflow_thread_ids);
DROP TABLE premise_asset_workflow_thread_ids;

CREATE TEMP TABLE workflows_premise_asset_backup AS SELECT * FROM workflows;
CREATE TEMP TABLE workflow_steps_premise_asset_backup AS SELECT * FROM workflow_steps;
CREATE TEMP TABLE workflow_events_premise_asset_backup AS SELECT * FROM workflow_events;
CREATE TEMP TABLE workflow_awaits_premise_asset_backup AS SELECT * FROM workflow_awaits;
CREATE TEMP TABLE llm_logs_premise_asset_backup AS
...
CREATE TABLE workflows (
    ...
    CONSTRAINT workflows_kind_check CHECK (kind IN ('yolo_project_initialization', 'comic_section_image_generation', 'comic_storyboard_generation', 'story_chapter_generation', 'story_chapter_batch_plan')),
...
SELECT
    ...
    idempotency_key,provider_uuid,model,model_source,current_step_key,error_code,
    error_message,cancel_requested_at,started_at,completed_at,created_at,updated_at
FROM workflows_premise_asset_backup;
...
SELECT
    ...
    task_uuid,resource_uuid,input_json,output_json,error_code,error_message,
    started_at,completed_at,created_at,updated_at
FROM workflow_steps_premise_asset_backup;
...
FROM workflow_events_premise_asset_backup;
...
SELECT
    ...
    error_message,http_status,provider_error_code,provider_request_id,created_at,
    completed_at,request_payload,response,cached_input_tokens,input_characters,output_characters
FROM llm_logs_premise_asset_backup;
...
SELECT
    id,uuid,workflow_id,chat_thread_id,chat_turn_id,chat_run_id,tool_execution_id,
    status,river_job_id,created_at,ready_at,resumed_at,cancelled_at,updated_at
FROM workflow_awaits_premise_asset_backup;

DROP TABLE workflows_premise_asset_backup;
DROP TABLE workflow_steps_premise_asset_backup;
DROP TABLE workflow_events_premise_asset_backup;
DROP TABLE workflow_awaits_premise_asset_backup;
DROP TABLE llm_logs_premise_asset_backup;
