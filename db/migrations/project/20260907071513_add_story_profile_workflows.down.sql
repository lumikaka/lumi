...
CREATE TEMP TABLE story_profile_workflow_thread_ids AS
SELECT thread_id FROM workflows
WHERE kind IN ('story_profile_generation','story_profile_from_chapters') AND thread_id IS NOT NULL
AND EXISTS(SELECT 1 FROM chat_threads WHERE id=workflows.thread_id AND thread_type='workflow');
DELETE FROM workflows WHERE kind IN ('story_profile_generation','story_profile_from_chapters');
DELETE FROM chat_threads WHERE id IN (SELECT thread_id FROM story_profile_workflow_thread_ids);
DROP TABLE story_profile_workflow_thread_ids;

CREATE TEMP TABLE workflows_story_profile_backup AS SELECT * FROM workflows;
CREATE TEMP TABLE workflow_steps_story_profile_backup AS SELECT * FROM workflow_steps;
CREATE TEMP TABLE workflow_events_story_profile_backup AS SELECT * FROM workflow_events;
CREATE TEMP TABLE workflow_awaits_story_profile_backup AS SELECT * FROM workflow_awaits;
CREATE TEMP TABLE llm_logs_story_profile_backup AS
SELECT * FROM llm_logs WHERE workflow_id IS NOT NULL OR workflow_step_id IS NOT NULL;

CREATE TEMP TABLE llm_cost_backfill_items_story_profile_backup AS
SELECT * FROM llm_cost_backfill_items WHERE llm_log_id IN (SELECT id FROM llm_logs_story_profile_backup);
...
CREATE TABLE workflows (
    ...
    CONSTRAINT workflows_kind_check CHECK (kind IN ('yolo_project_initialization', 'premise_asset_generation', 'comic_section_image_generation', 'comic_image_generation_batch', 'comic_storyboard_generation', 'story_chapter_generation', 'story_chapter_batch_plan')),
...
SELECT
    ...
    idempotency_key,provider_uuid,model,model_source,current_step_key,error_code,
    error_message,cancel_requested_at,started_at,completed_at,created_at,updated_at
FROM workflows_story_profile_backup;
...
SELECT
    ...
    task_uuid,resource_uuid,input_json,output_json,error_code,error_message,
    started_at,completed_at,created_at,updated_at
FROM workflow_steps_story_profile_backup;
...
FROM workflow_events_story_profile_backup;

-- Preserve every log field, including pricing snapshots, and the dependent
-- backfill records introduced before this migration.
INSERT INTO llm_logs SELECT * FROM llm_logs_story_profile_backup;
INSERT INTO llm_cost_backfill_items SELECT * FROM llm_cost_backfill_items_story_profile_backup;
...
SELECT
    id,uuid,workflow_id,chat_thread_id,chat_turn_id,chat_run_id,tool_execution_id,
    status,river_job_id,created_at,ready_at,resumed_at,cancelled_at,updated_at
FROM workflow_awaits_story_profile_backup;

DROP TABLE workflows_story_profile_backup;
DROP TABLE workflow_steps_story_profile_backup;
DROP TABLE workflow_events_story_profile_backup;
DROP TABLE workflow_awaits_story_profile_backup;
DROP TABLE llm_logs_story_profile_backup;

DROP TABLE llm_cost_backfill_items_story_profile_backup;
