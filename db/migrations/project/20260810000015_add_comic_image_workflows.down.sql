...
WHERE id IN (
    ...
    WHERE kind = 'comic_section_image_generation' AND thread_id IS NOT NULL
);
DELETE FROM workflows WHERE kind = 'comic_section_image_generation';

CREATE TEMP TABLE llm_logs_workflow_migration_backup AS SELECT * FROM llm_logs;
DROP TABLE llm_logs;
CREATE TEMP TABLE workflow_events_migration_backup AS SELECT * FROM workflow_events;
DROP TABLE workflow_events;
CREATE TEMP TABLE workflow_steps_migration_backup AS SELECT * FROM workflow_steps;
...
INSERT INTO workflows_previous (
    id, uuid, project_id, thread_id, kind, title, status, input_version,
    input_snapshot, idempotency_key, provider_uuid, model, current_step_key,
...
INSERT INTO workflow_steps SELECT * FROM workflow_steps_migration_backup;
DROP TABLE workflow_steps_migration_backup;
...
INSERT INTO workflow_events SELECT * FROM workflow_events_migration_backup;
DROP TABLE workflow_events_migration_backup;
...
INSERT INTO llm_logs SELECT * FROM llm_logs_workflow_migration_backup;
DROP TABLE llm_logs_workflow_migration_backup;
