...
DELETE FROM production_task_runs
WHERE uuid IN (SELECT task_uuid FROM comic_exports WHERE format = 'pdf');
DELETE FROM comic_exports WHERE format = 'pdf';

CREATE TABLE comic_exports_zip_only (
...
INSERT INTO comic_exports_zip_only (
...
ALTER TABLE comic_exports_zip_only RENAME TO comic_exports;
