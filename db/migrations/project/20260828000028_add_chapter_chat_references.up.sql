...
CREATE TABLE chat_context_references_next (
    ...
    CONSTRAINT chat_context_references_type_check CHECK (resource_type IN ('file', 'premise_asset', 'chapter', 'comic_section')),
    ...
    CONSTRAINT chat_context_references_target_check CHECK (
        ...
        (resource_type='chapter' AND file_id IS NULL AND premise_asset_id IS NULL AND comic_section_id IS NULL) OR
        (resource_type='comic_section' AND file_id IS NULL AND premise_asset_id IS NULL)
    )
);

INSERT INTO chat_context_references_next (
    id, chat_item_id, follow_up_id, position, resource_type, resource_uuid,
    snapshot_json, file_id, premise_asset_id, comic_section_id,
...
ALTER TABLE chat_context_references_next RENAME TO chat_context_references;
