...
WITH mapped AS (
    SELECT
        ...
        CASE WHEN prompt_group = 'runtime' THEN 'agent' ELSE prompt_group END AS mapped_group,
        CASE WHEN prompt_group = 'runtime' THEN '__lumi_runtime__.' || prompt_key ELSE prompt_key END AS mapped_key,
        version_no, prompt, prompt_hash,
        CASE WHEN source_type IN ('project_created', 'migration') THEN 'manual_edit' ELSE source_type END AS mapped_source_type,
        created_at
...
SELECT
    ...
    mapped_version_no, prompt, prompt_hash, mapped_source_type, created_at
FROM numbered;
