...
ALTER TABLE project_creation_sessions RENAME TO project_creation_sessions_without_references;
...
CREATE TABLE project_creation_sessions (
    ...
    CONSTRAINT project_creation_sessions_status_check CHECK (status IN ('pending','creating_project','awaiting_references','creating_conversation','active','failed','cancelled')),
...
SELECT
    ...
    planned_root_path, recent_project_id, thread_uuid, turn_uuid, error_code,
    error_message, attempt_count, created_at, updated_at, completed_at, failed_at
FROM project_creation_sessions_without_references;

DROP TABLE project_creation_sessions_without_references;
